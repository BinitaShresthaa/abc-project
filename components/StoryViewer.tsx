...
export default function StoryViewer({
  ...
  contained?: boolean; // true = small dimmed-overlay card (dashboard preview); false = full-bleed fullscreen (public page)
}) {
  ...
  {photos.map((_, i) => (
    <div key={i} className="h-1 flex-1 overflow-hidden rounded-full bg-white/30">
      <div
        ...
        style={{
          width: i < photoIndex ? "100%" : i === photoIndex ? `${fill}%` : "0%",
          transition: i === photoIndex ? `width ${DURATION}ms linear` : "none",
        }}
      />
    </div>
  ))}
...
/>
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-#0E76BD/60 backdrop-blur-sm p-4">
      {card}
    </div>
  );
}