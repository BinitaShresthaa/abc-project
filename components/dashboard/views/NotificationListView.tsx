...
import RowMenu from "@/components/dashboard/table/RowMenu";
...
export default function NotificationListView() {
  ...
  // Only unread notifications are selectable at all — a read one has
  // nothing left to "mark as read", so it should never show a checkbox
  // or stay in the selected set.
  const selectableIds = new Set(mockNotifications.filter((n) => !n.read).map((n) => n.id));
  ...
  function openNotification(n: FeedbackNotification) {
    ...
    if (!n.read) {
      ...
      // If it was selected, drop it from the selection too — it's no
      // longer an unread item that a bulk "mark as read" applies to.
      setSelected((prev) => {
        const next = new Set(prev);
        next.delete(n.id);
        return next;
  ...
  function handleMarkOneRead(n: FeedbackNotification) {
    markNotificationRead(n.id);
    setSelected((prev) => {
      const next = new Set(prev);
      next.delete(n.id);
  ...
  function rowActionsFor(n: FeedbackNotification): RowAction<FeedbackNotification>[] {
    // A read notification has no actions left — no menu at all for it.
    if (n.read) return [];
    return [{ label: "Mark as read", onSelect: () => handleMarkOneRead(n), variant: "primary" }];
  }
      ...
      {mockNotifications.length === 0 ? (
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {mockNotifications.map((n) => (
            ...
            >
              {/* Checkbox only exists for unread items — a read notification
                  has nothing to select for a bulk "mark as read" action. */}
              {selectableIds.has(n.id) ? (
                <input
                  type="checkbox"
                ...
                <span className="mt-1 h-4 w-4 shrink-0" /> // keeps row alignment consistent
              )}
              ...
              <RowMenu row={n} actions={rowActionsFor(n)} />
            </div>
          ))}
        </div>
      )}
